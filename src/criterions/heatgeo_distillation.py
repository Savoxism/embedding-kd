...
def scale_weights_from_law(
    diffusion_scales: Sequence[int], weight_exponent: float = 0.0
) -> tuple[float, ...]:
    """omega_r = r**-weight_exponent over the sorted scale set.

    On the dyadic grid t_j = 2^j the spacing is Delta t_j = t_j, so these
    weights are the Riemann discretization of the scale measure dt/t^(1+e).
    e = 0 (default) gives equal weight per octave -- the log-uniform measure
    dt/t, the convention of heat-kernel signatures (HKS, SI-HKS, NetLSD) and
    of unweighted dyadic diffusion wavelets. e = 1 reproduces the historical
    (1, 1/2, 1/4) and is kept reachable only for comparison runs.

    The single source of truth for the weight law: the criterion derives its
    loss weights from it, and the distiller feeds the same values to the graph
    builder (pool selection) and the candidate sampler (mixture sampling).
    """
    ...
    return tuple(float(r) ** -float(weight_exponent) for r in scales)
...
class HeatGeoDistillation(nn.Module):
    ...
    **The scale measure.** Per-scale weights are not accepted either; they
    are generated by one exponent:

        omega_r  proportional to  r**-weight_exponent

    On the dyadic grid t_j = 2^j the spacing is Delta t_j = t_j, so omega_r =
    r^-e discretizes the scale measure dt/t^(1+e). The default e = 0 -- equal
    weight per octave -- is the scale-invariant measure dt/t: logarithmic time
    sampling, the convention of heat-kernel signatures (HKS, SI-HKS, NetLSD)
    and of unweighted dyadic diffusion wavelets. e = 1 reproduces the
    historical (1, 1/2, 1/4) -- the measure dt/t^2, uniform in the probed
    frequency band 1/t -- and is kept reachable only for comparison runs.

    The direct scale's pre-normalization weight is fixed at 1 against the
    normalized diffusion family -- the objective splits evenly between absolute
    calibration (direct) and neighbourhood ranking (graph scales), independent
    of e. It is one member of the ladder, not an optional term; ablate it by
    passing ``teacher_embeddings=None``. Passing any removed knob
    (``scale_temps``, ``walk_temp``, ``direct_student_temp``,
    ``broad_scale_temps``, ``direct_temp``, ``direct_weight``,
    ``student_temp``, ``scale_weights``) raises rather than being silently
    absorbed.
    """
    ...
    _REMOVED_KNOBS = {
        ...
        "scale_weights": (
            "generated by the law omega_r = r**-weight_exponent; the default "
            "weight_exponent = 0 gives equal weight per dyadic octave (the "
            "log-uniform scale measure of heat-kernel signatures)"
        ),
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # omega_r = r^-e over the sorted scales: the dyadic discretization of
        # the scale measure dt/t^(1+e). The default e = 0 -- equal weight per
        # octave -- is the log-uniform measure dt/t, the convention of
        # heat-kernel signatures; e = 1 reproduces the historical (1, 1/2, 1/4)
        # and is kept reachable only for comparison runs. The direct scale
        # keeps pre-normalization weight 1 against this *normalized* family in
        # forward(): the objective splits evenly between absolute calibration
        # and neighbourhood ranking, independent of e.
        weights = torch.tensor(
            scale_weights_from_law(scales, self.weight_exponent),
            dtype=torch.float32,
        )
