...
(ii) The characterization in (i) involves $c^S_{ij}$ only for $j\in\mathcal{B}\setminus\{i\}$, and $\mathcal{S}_i^{T}$ is the union of these sets over the batches containing $i$. Scores to columns outside $\mathcal{S}_i^{T}$ appear in no constraint and may be set arbitrarily. If two batches $\mathcal{B},\mathcal{B}'$ containing $i$ share no element, no column is subject to both constraints, so $a_i^{\mathcal{B}}$ and $a_i^{\mathcal{B}'}$ are unrelated; if they share a column $j$, then both equal $c^S_{ij}-c^T_{ij}$ and coincide. \qed

\begin{remark}[The defect is identifiability, not bias]
The in-batch teacher target $p^{T,\mathcal{B}}_i$ is a poor estimator of the teacher's ambient distribution over the corpus, but this does not by itself impugn the objective: the student is restricted to the same columns, so by part (i) the exact solution $c^S=c^T+a_i$ drives the loss to zero on every batch and the two restrictions cancel. The failure is that almost nothing is constrained, which Proposition~\ref{prop:coverage}(iii) quantifies.
\end{remark}
...
\subsection{Leakage control: why negatives bound $\delta_S$}
...
\paragraph{Approximate identification (Remark~\ref{rem:approxgauge}).}
