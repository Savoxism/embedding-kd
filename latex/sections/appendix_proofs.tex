...
The teacher's transition row is itself a softmax of teacher cosines at the graph temperature, $P^T_j(j')\propto\exp(c^T_{jj'}/\tau_g)$ on $\mathcal{M}_k^T(j)$, and restricting a softmax to a subset and renormalizing is the softmax over that subset. Hence $P^T_j|_{\Omega^w_j}=\mathrm{softmax}_{\Omega^w_j}(c^T_{j\cdot}/\tau_g)$, and the walk term of Eq.~\ref{eq:lwalk} vanishes at $j$ exactly when $\mathrm{softmax}_{\Omega^w_j}(c^S_{j\cdot}/\tau_w)=\mathrm{softmax}_{\Omega^w_j}(c^T_{j\cdot}/\tau_g)$. With $\tau_w=\tau_g$ the log-ratio argument above applies verbatim on $\Omega^w_j$, giving $c^S_{jv}=c^T_{jv}+a_j$ for all $v\in\Omega^w_j$.
