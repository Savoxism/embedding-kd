...
Anchor-wise objectives are not independent because their local geometries overlap. RIPPLE closes the sampled support under teacher transition rows, reusing already encoded candidates rather than sampling additional row centers. Let $\mathcal P_B$ be the deduplicated shared pool and let $\mathcal S_i\subseteq\mathcal P_B$ contain the columns with positive mass in anchor $i$'s realized diffusion targets. Thus $\mathcal S_i$ excludes hard and uniform negatives. The auxiliary row centers are
\begin{equation}
\mathcal J_B
=\left(\bigcup_{i\in B}\mathcal S_i\right)\setminus B,
\end{equation}
where removing all batch anchors avoids duplicating the one-hop term already present in $\mathcal L_{\mathrm{rel}}$. For $j\in\mathcal J_B$, define the exposed teacher-neighbor support and its retained mass as
\begin{equation}
\Omega_j^{\mathrm{row}}
...
\end{equation}
The teacher row is restricted and renormalized as $\bar P_{j\ell}^{T,B}=P_{j\ell}^T/m_B(j)$, while the student row is
\begin{equation}
q_{j\ell}^{S,B}
...
\label{eq:row-loss}
\end{equation}
The exposed mass $m_B(j)$ is monitored but does not reweight the rows. Rows with fewer than two available teacher-neighbor columns are omitted because their restricted KL is uninformative. The complete objective is
\begin{equation}
