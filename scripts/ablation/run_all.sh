...
#   PLAN="components sensitivity" GPUS="0 1" bash scripts/ablation/run_all.sh
...
PLAN="${PLAN:-support components sensitivity}"
read -r -a PLAN_LIST <<< "${PLAN}"
LOG_DIR="${LOG_DIR:-runs/ablation/_dispatch/${EXPERIMENT_KEY:-paper_v1}}"
mkdir -p "${LOG_DIR}"
...
for group in "${PLAN_LIST[@]}"; do
    if [[ -z "${SCRIPT_OF[${group}]:-}" ]]; then
        echo "Unknown group '${group}'. Choose from: support components sensitivity" >&2
        exit 2
    fi
done
    ...
    if (( index > 0 && index % ${#GPU_LIST[@]} == 0 )); then
        for slot in "${!pids[@]}"; do
            if ! wait "${pids[${slot}]}"; then
                echo "[fail] ${names[${slot}]} (see ${LOG_DIR}/${names[${slot}]}.log)" >&2
                exit 1
            fi
        done
        pids=()
...
if [[ "${MAKE_HEATMAP:-0}" == "1" && "${DRY_RUN:-0}" != "1" ]]; then
    echo "[figure] geometry heatmap on GPU ${GPU_LIST[0]}"
    GPU="${GPU_LIST[0]}" bash "${SCRIPT_DIR}/heatmap.sh"
fi
