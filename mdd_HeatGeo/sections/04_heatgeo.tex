...
\begin{equation}
    ...
    \right),
\end{equation}
where $B$ is a mini-batch and $\omega_r$ is the scale weight. The scales now share the underlying similarity vector $\cos(s_i,\cdot)$ but not the distribution built from it, so \eqref{eq:collapse} no longer applies and \eqref{eq:floor} becomes a lower bound rather than an attained value. The mechanism is over-determination: $|\mathcal{R}|$ softmaxes at distinct temperatures impose $|\mathcal{R}|$ independent constraints on one $|C_i|$-vector of cosines, and satisfying them all requires reproducing the teacher's similarity \emph{gaps}, not merely its ranking. A single softmax pins $\cos(s_i,s_j)$ only up to a per-anchor additive constant, which is exactly the calibration that Spearman correlation on STS and any globally thresholded cosine retrieval depend on. In a controlled setting with free logits, minimizing the tied objective converges to the floor of \eqref{eq:floor} to four decimals, whereas untying $\tau_r$ reaches a value well below it.
...
\label{alg:heatgeo}
\begin{algorithmic}[1]
\REQUIRE Corpus $\mathcal{D}$, teacher $T$, student $S_{\theta}$, graph size $k$, scales $\mathcal{R}$, temperatures $\{\tau_r\}$, pool size $P$, candidate size $C$
...
\FOR{each training epoch $e$}
    ...
    \FOR{each mini-batch $B$}
        ...
        \STATE Compute $p_{i,r}^S$ at temperature $\tau_r$ for each $r\in\mathcal{R}$
