...
class KnowledgeDistiller:
            ...
            with autocast("cuda", enabled=torch.cuda.is_available()):
                ...
                if hasattr(self, "kd_student_layers"):
                    ...
                    for l_idx, criterion in enumerate(self.heatgeo_criterions):
                        ...
                        # getattr, not cfg.lambda_heatgeo: the knob was dropped from
                        # HeatGeoConfig because only this branch reads it, and this
                        # branch needs both kd_*_layers defined to run at all.
    ...
    def train_epoch(self, epoch: int):
        ...
        if hasattr(self.train_ds, "set_epoch"):
            # Keeps its getattr fallback on purpose: train_epoch runs for every
            # method, and only the HeatGeo config declares this knob. The gate is
            # the dataset's own set_epoch, not the method name, so a direct read
            # here would raise for any other method that grows a resampling dataset.
            sampling_epoch = (
